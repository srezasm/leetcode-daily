...
class Solution {
   public:
    int dfs(int i, vector<int>& children, vector<int>& cookies, int k, int zeroCount) {
        ...
        int answer = INT_MAX;
        for (int j = 0; j < k; ++j) {
            zeroCount -= children[j] == 0 ? 1 : 0;
            children[j] += cookies[i];

            answer = min(answer, dfs(i + 1, children, cookies, k, zeroCount));
